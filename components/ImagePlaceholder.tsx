...
function ImagePlaceholderComponent(props: NodeViewProps) {
	...
	const {
		...
		onUpload: (imageUrl) => {
			...
			}).deleteNode("image-placeholder").run();
			handleRemove();
			setIsExpanded(false);
		},
	});
	...
	const handleInsertEmbed = (e: FormEvent) => {
		...
		if (url) {
			editor.chain().focus().setImage({ src: url, alt: altText }).deleteNode("image-placeholder").run();
			setIsExpanded(false);
			setUrl("");
			setAltText("");
		}
			...
			<div className="relative">
				{!isExpanded ? (
					<div
						...
						className={cn(
							"group relative flex cursor-pointer flex-col items-center gap-4 rounded-lg border-2 border-dashed p-8 transition-all hover:bg-muted",
							selected && "border-primary bg-primary/5",
							isDragActive && "border-primary bg-primary/5",
							error && "border-destructive bg-destructive/5"
						)}
					>
						<div className="rounded-full bg-background p-4 shadow-sm transition-colors group-hover:bg-muted">
